  ...
  <f7-page @page:afterin="onPageAfterIn" @page:beforeout="onPageBeforeOut" class="thing-details-page">
    ...
    <f7-tabs>
      <f7-tab id="thing" :tab-active="currentTab === 'thing'">
        ...
        <!-- Actions -->
        <f7-block class="block-narrow" v-if="ready && !error" v-for="actionGroup in configActionsByGroup" :key="actionGroup.group.name">
          <f7-col>
            <f7-block-title class="parameter-group-title">{{ actionGroup.group.label }}</f7-block-title>
  ...
  methods: {
    ...
    getBindingActions (configDescriptionsResponse) {
      ...
      return map(groupBy(bindingActions, 'groupName'), (gActions,gName) => {
              return { group: configDescriptionsResponse.parameterGroups.find((pg) => pg.name === gName), 
